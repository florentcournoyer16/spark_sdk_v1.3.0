...
/* PUBLIC FUNCTIONS ***********************************************************/
void pairing_timer_init(swc_hal_t *swc_hal, uint16_t timeout_sec)
{
    /* Get the handle from the board's hal to access the free running timer. */
    hal = swc_hal;

    /* Get the tick frequency. */
    local_tick_frequency_hz = hal->radio_hal->tick_frequency_hz;
