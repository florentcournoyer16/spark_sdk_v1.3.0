...
void pairing_wireless_init(pairing_cfg_t *pairing_cfg, swc_role_t network_role)
{
    ...
    swc_init(core_cfg, local_pairing_cfg.hal, &err);
    if (err != SWC_ERR_NONE) {
    ...
    swc_node_add_radio(node, radio_cfg, local_pairing_cfg.hal, &err);
    if (err != SWC_ERR_NONE) {
        handle_wireless_error();
        return;
    }
    ...
    swc_connection_cfg_t coord_to_node_conn_cfg = {
        ...
        .cca_settings.threshold     = PAIRING_CCA_THRESHOLD,
        .cca_settings.retry_time    = PAIRING_CCA_RETRY_TIME,
        .cca_settings.try_count     = PAIRING_CCA_TRY_COUNT,
        .throttling_enabled         = false,
        .rdo_enabled                = false,
        .fallback_enabled           = false,
    };
    coord_to_node_conn = swc_connection_init(node, coord_to_node_conn_cfg, local_pairing_cfg.hal, &err);
    if (err != SWC_ERR_NONE) {
        handle_wireless_error();
        return;
    }
    ...
    node_to_coord_conn = swc_connection_init(node, node_to_coord_conn_cfg, local_pairing_cfg.hal, &err);
    if (err != SWC_ERR_NONE) {
        handle_wireless_error();
        return;
    }
...
static void store_radio_serial_number(void)
{
    ...
    local_radio_serial_number = swc_node_get_radio_serial_number(node, &err);
    if (err != SWC_ERR_NONE) {
        handle_wireless_error();
        return;
    }
