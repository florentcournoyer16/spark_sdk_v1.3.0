...
{
    wps_mac_t *mac           = wps_mac;
    link_phase_t *link_phase = &mac->auto_connection->link_phase;
    ...
    if (is_phase_data_valid(&mac->phase_data)) {
        if (link_phase_add_data(link_phase, mac->phase_data.last_local_phases_info,
            ...
            mac->config.callback_auto.callback = mac->auto_connection->ranging_data_ready_callback;
            mac->config.callback_auto.parg_callback =
                mac->auto_connection->ranging_data_ready_parg_callback;
