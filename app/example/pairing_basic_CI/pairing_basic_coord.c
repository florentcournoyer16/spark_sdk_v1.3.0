...
/* The discovery list include the coordinator and the node devices. */
...
static void app_swc_configuration(pairing_assigned_address_t *pairing_assigned_address, swc_error_t *err)
{
    ...
    /*
     * The assigned address can be used directly since only
     * the coordinator and node devices are in the network.
     */
    coordinator_address = pairing_assigned_address->coordinator_address;
    node_address = pairing_assigned_address->node_address;

    /*
     * Alternatively, the pairing discovery list could be used.
     * However, the discovery list is more suitable if there are
     * more than two devices in the network.
     *
     * Here is an example when using the discovery list:
     * coordinator_address = pairing_discovery_list[DEVICE_ROLE_COORDINATOR].node_address;
     * node_address = pairing_discovery_list[DEVICE_ROLE_NODE].node_address;
     *
     * where
     *   DEVICE_ROLE_COORDINATOR = 0
     *   DEVICE_ROLE_NODE = 1.
     */
...
static void enter_pairing_mode(void)
{
    ...
    {
        /* A pairing error is critical and must not be ignored. */
        while (1)
            ;
    }
